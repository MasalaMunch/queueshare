...
const eventuallyUpdate = require(`./eventuallyUpdate.js`);
...
(async () => {
    ...
    const clientJs = await ClientJs();

    const fsPortPath = path.join(folder, `port`);

    const fsPort = new FsJson(fsPortPath, Port.Valid, Port());

    const port = fsPort.Value();
    ...
    app.post(`/pid`, emitActivityMiddleware, (req, res) => {
        ...
        log(`A client requested a restart.`, restartMsg);
